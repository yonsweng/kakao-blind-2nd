...
while True:
    ...
    ori_calls = oncalls['calls']

    candi_floors_list = [{1, 13}, set(range(1, 13)), set(range(13, 26))|{1}, set(range(1, 26))]
    for elevator, candi_floors in zip(oncalls['elevators'], candi_floors_list):
        calls = [call for call in ori_calls if call['start'] in candi_floors and call['end'] in candi_floors]
        ...
        if are_ends_this_floor(elevator['passengers'], elevator['floor']):  # 내릴 사람이 있는 경우
            ...
            if elevator['status'] == 'UPWARD':
                if elevator['floor'] == max(candi_floors):
                    commands[elevator['id']]['command'] = 'STOP'
                else:
                    commands[elevator['id']]['command'] = 'UP'
            elif elevator['status'] == 'DOWNWARD':
                if elevator['floor'] == min(candi_floors):
                    commands[elevator['id']]['command'] = 'STOP'
                else:
                    commands[elevator['id']]['command'] = 'DOWN'
            elif elevator['status'] == 'STOPPED':
                if len(calls) > 0 and len(elevator['passengers']) < 8:
                    if are_starts_more_above(calls, elevator['floor']):
                        commands[elevator['id']]['command'] = 'UP'
                    else:
                        commands[elevator['id']]['command'] = 'DOWN'
